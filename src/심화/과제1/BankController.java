...
//        account.pwd = pwd;
    ...
    boolean pullMoney(String accountNum, String pwd, int balance) {
        int money = 0;
    ...
    // 4. 잔고 체크
    // 계좌, 비번 같은지 체크
    // 출력 -> 전체 배열이 아니라서 배열 대신 int 반환해야함

    int balanceCheck(String accountNum, String pwd) {
        for (int i = 0; i <= accounts.length - 1; i++) {
            //  if문으로 계좌번호, 비번이 기존 인덱스 요소와 맞는지 비교
        ...
        return -1;  // return 값이 0 이상일 때 출력할 거임
    }
}