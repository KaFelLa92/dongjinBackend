...
*/
        ...
        // (5)
        // boardService1_변수활용 : 게시물 3개 -> 변수 6개 , 게시물 100개 -> 변수 200개
        // boardService2_배열활용 : 게시물 100개 -> 변수 2개
        // boardService3_객체활용 : 게시물 100개 -> 변수 2개(객체화) -> 변수 1개

        // 100개의 저장 공간을 갖춘 boards 배열
        // String[] contents = new String[100];
        // String[] writers = new String[100]; 속성 다양해지면 배열 개수 많아지므로, 하나의 클래스로 설계
        // 설계한 클래스/타입으로 배열 선언.
        Board[] boards = new Board[100];
        // boards 변수에 Board 객체 100개 저장 가능한 배열
        // 변수 1개, 배열1개, 객체100개 저장공간
        boolean check = false;
        // for ( ; ;) 무한루프 프로그램
        for (; ; ) {
            // scanner로 기능 입력받기
            ...
            if (select == 1) {
                ...
                for (int i = 0; i <= boards.length - 1; i++) {
                    if (boards[i] == null) { // i번째 요소가 비어있으면
                        boards[i] = board; // 생성한 객체 대입
                        check = true; // 상태 변경
                        break; // 반복문 종료
                    } // if end
                }// for end
            ...
            if (select == 2) {
                ...
                // 배열내 요소(Board 객체)가 비어있지 않으면( != null) board 객체 출력
                for (int i = 0; i <= boards.length - 1; i++) {
                    Board board = boards[i];    // board는 boards의 인덱스값.
                    if (board != null) {
