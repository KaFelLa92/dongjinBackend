...
public class DiningService3 {   // class s
    public static void main(String[] args) { // main s
        ...
        main 함수는 사용자 입출력(View)을, Controller는 데이터 처리(Controller)를 담당하도록
        역할을 분담하여 보다 체계적인 프로그램을 구현합니다.
         */
...
*/
        ...
        // 객체 생성
        WaitingController control = new WaitingController();
        ...
        for (; ; ) {
            ...
            if (select == 1) {
                ...
                // addWaiting 메소드 호출
                boolean result = control.addWaiting(phone, count); // 애드웨이팅의 넘버, 인원수를 result에 저장
                // if문으로 boolean 결과 true, false 확인
                if (result == true) {
            ...
            if (select == 2) {
                ...
                // getWaitingList가 반환된 배열 불러오기
                Waiting[] waitings = control.getWaitingList();
                //  for문 돌려서 반복 출력, if문으로 != null 체크,
