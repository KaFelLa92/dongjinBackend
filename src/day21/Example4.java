    ...
    public static void main(String[] args) {
        /*
            [ 스레드풀 ]
                ...
                2. 목적 : 작업이 들어올때마다 새로운 스레드가 아닌 기존 스레드 이용한다.
                ...
                4. 구조 : 작업의 요청을 '큐'라는 자료구조로 배치하여 순서대로 스레드풀에서 대기중인 스레드에게 작업 배정
                    * 큐? 자료의 데이터들을 요청 순서대로 처리하는 방법
                    ...
                    ----------------------------------------
                5. 생활속예시 : 콜센터 , 유재석이 콜센터에 전화하면 대기중인 직원1(스레드)가 존재하면 처리한다. 아니면 대기
                                            스레드풀
                                              직원1( 스레드 )
                    콜요청 --------->          직원2( 스레드 )
                                              직원3( 스레드 )
        ...
        for (int i = 1; i <= 10; i++){
            ...
            Runnable runnable = new Runnable(){
                @Override
                public void run() { // 싱크로나이즈드 쓰면 1부터 10까지 순번대로 처리함
                    try{
                        Thread.sleep(2000);
                    } catch (Exception e){
        ...
        // 스레드풀에 저장한 스레드수가 3개라서, 3개씩 출력.

        // 스레드 실행 순서는 지멋대로임
        // 웨이팅 걸어주는 프로그램에 활용 가능하다. (한번에 몇만명 넘게 들어올 수 있는 곳에서)
