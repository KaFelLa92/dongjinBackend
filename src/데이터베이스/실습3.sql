...
select title , author from books where author not in ('이');
select title , author from books where not author = '이';
...
select title, pub_date from books order by pub_date asc limit 4 , 3;
